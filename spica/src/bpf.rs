...
use aya::{
    ...
    programs::{Lsm, PerfEvent, TracePoint, perf_event},
...
impl BpfRuntime {
    ...
    pub fn load(
        ...
        // Helper function to load and attach sched_switch as a traditional tracepoint
        fn load_and_attach_sched(bpf: &mut Ebpf) -> Result<()> {
            let prog: &mut TracePoint = bpf.program_mut("spica_sched").unwrap().try_into()
        ...
        // 2. Attach observation channels in quick succession to minimize startup asymmetry
        load_and_attach_sched(&mut bpf)?;
        load_and_attach_nmi(&mut bpf)?;

        // 3. Attach remaining programs
        attach_watchdog(&mut bpf)?;
        attach_lsm_if_attached(&mut bpf, lsm_attached)?;

        // 4. Pin watchdog map to BPF-FS so it outlives a SIGKILL.
        bpf.map("sc_wd").unwrap().pin(WATCHDOG_PIN)?;

        // 5. Take ring buffers into AsyncFd. take_map moves them out of `bpf`
